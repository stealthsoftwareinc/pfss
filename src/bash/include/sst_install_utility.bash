...
sst_install_utility() {
  ...
  sst_get_distro >/dev/null || sst_err_trap "$sst_last_command"
    ...
    eval $utility'() {
      ...
      if [[ "${sst_utility_overrides['$utility']-}" ]]; then
        ...
        "'\'' "$@"'\'' || {
          ...
          #
          # If we get here, errexit is suspended. We need to unset
          # sst_bash_command so we do not override the failed command
          # printed by a later invocation of the ERR trap, and we need
          # to call sst_set_exit again to maintain the exit status.
          #
