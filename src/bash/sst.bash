...
unset sst_trap_entry_status

declare    sst_root_tmpdir
declare    sst_trap_entry_status
...
#-----------------------------------------------------------------------
#
# SST_PRELUDE_DEPTH cannot be readonly because sst_exec needs to modify
# it.
#
...
for sst_file in "$sst_sstdir"/include/**/*.bash; do
  sst_name=${sst_file##*/}
...
if [[ "${SST_NDEBUG=0}" != [01] ]]; then
  sst_x=$(sst_smart_quote "$SST_NDEBUG")
  sst_barf "Invalid SST_NDEBUG value: $sst_x"
fi
...
sst_exit_trap() {
  ...
  #---------------------------------------------------------------------
  #
  # Note that any code in the script may need to access the temporary
  # directory, so deleting the temporary directory should be the last
  # thing the script does. Doing it here does precisely that.
  #

  rm -f -r "$sst_root_tmpdir" || :
