...
/* 
* TODO: change eval all to have an additional parameter to store the dot product
...
int main(void) {
  ...
  {
    pfss_op op = pfss_dot_product; // choose lookup query operation
    uint32_t y1
        [domain_max]; // initialize array of zeros to be looked up by the test query
    uint32_t y2
        [domain_max]; // initialize array of zeros to be looked up by the test query
    y1[domain_val] = 1; // set the domain slot value in one the arrays to be 1
    uint32_t scalar_res1;
    uint32_t scalar_res2;
    {
      pfss_status const s =
          pfss_eval_all_32(p_context, key1, y1, &scalar_res1, op);
      if (s != PFSS_OK) {
        exit_status = TEST_EXIT_ERROR;
        goto cleanup;
      }
    ...
    {
      pfss_status const s =
          pfss_eval_all_32(p_context, key2, y2, &scalar_res2, op);
      if (s != PFSS_OK) {
        exit_status = TEST_EXIT_ERROR;
        goto cleanup;
      }
    ...
    // check correctness of lookup query: the sum of the scalar res's should equal
    // the sum of the arrays at the secret domain value
    if (scalar_res1 + scalar_res2 != 1) {
        exit_status = TEST_EXIT_ERROR;
        goto cleanup;
      }
  }
